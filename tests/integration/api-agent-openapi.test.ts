...
describe('GET /api/agent/openapi.json', () => {
  it('renvoie une spec OpenAPI publique', async () => {
    ...
    expect(body.paths['/api/agent/journal/{date}']).toBeTruthy()
    expect(body.paths['/api/agent/history']).toBeTruthy()
    expect(body.paths['/api/agent/week-priorities']).toBeTruthy()
    expect(body.paths['/api/agent/week-priorities/{id}']).toBeTruthy()
  })
})
