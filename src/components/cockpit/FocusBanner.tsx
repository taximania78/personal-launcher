...
export async function FocusBanner() {
  try {
    ...
    return <FocusBannerView state={resolveFocusBannerState(todo, journal)} todoId={todo?.id ?? null} />
  } catch {
    return <FocusBannerView state={{ kind: 'unset', deepWork: false }} todoId={null} />
  }
}
